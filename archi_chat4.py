...
class PricingPlanChatbot:
    ...
    def setup_chatbot_chain(self):
        ...
        # 시스템 프롬프트 정의
        system_prompt = """
        당신은 통신사 고객 상담원입니다. 사용자와 자연스럽고 친근하게 대화하며, 필요할 때만 요금제를 추천해주세요.

        **현재 사용자: {current_user}**

        **대화 지침:**
        1. 사용자와 자연스럽고 친근하게 대화하세요
        2. 인사, 일상 대화, 질문 등에 자연스럽게 응답하세요
        3. 사용자가 요금제에 대해 구체적으로 문의하거나 추천을 요청할 때만 요금제를 추천하세요
        4. 이전 대화 내용을 기억하고 연관성 있게 대화하세요

        **요금제 추천 시에만 다음 형식을 사용하세요:**
        ```
        **추천 요금제:** [요금제명]
        ...
        **요금제 추천이 아닌 일반 대화에서는 위 형식을 사용하지 마세요.**
    ...
    def is_plan_recommendation_request(self, user_input: str) -> bool:
        """사용자 입력이 요금제 추천 요청인지 판단"""
        plan_keywords = [
            '요금제', '추천', '플랜', '가입', '바꾸고', '바꿔', '변경', 
            '적합한', '맞는', '좋은', '저렴한', '싼', '데이터', '통화', 
            '월 요금', '예산', '가격', '비용', '할인', '프로모션'
        ]
        
        recommendation_keywords = [
            '추천해', '추천해줘', '추천해주세요', '추천 부탁', '어떤 게', 
            '뭐가 좋을까', '어떤 걸', '골라줘', '선택해줘', '알아봐줘'
        ]
        
        user_input_lower = user_input.lower()
        
        # 요금제 관련 키워드가 포함되어 있거나
        has_plan_keyword = any(keyword in user_input_lower for keyword in plan_keywords)
        
        # 추천 관련 키워드가 포함되어 있는 경우
        has_recommendation_keyword = any(keyword in user_input_lower for keyword in recommendation_keywords)
        
        return has_plan_keyword or has_recommendation_keyword
    ...
    def chat(self, user_input: str) -> str:
        ...
        try:
            ...
            # 대화 기록에 추가 (요금제 추천 여부도 함께 저장)
            conversation_entry = {
                "timestamp": datetime.now().isoformat(),
                "human": user_input,
                "ai": ai_response,
                "is_plan_recommendation": self.is_plan_recommendation_request(user_input)
            }
    ...
    def get_user_statistics(self) -> Dict[str, Any]:
        ...
        try:
            # 요금제 추천 대화 수 계산
            plan_recommendation_count = sum(1 for conv in self.chat_history 
                                          if conv.get('is_plan_recommendation', False))
            ...
            if memory_file.exists():
                ...
                return {
                    'username': self.current_user,
                    'total_conversations': len(self.chat_history),
                    'plan_recommendations': plan_recommendation_count,
                    'general_conversations': len(self.chat_history) - plan_recommendation_count,
...
def main():
    ...
    PRICING_DATA_FILE = os.getenv("PRICING_DATA_FILE", "plans_manual.txt")  # 기본값 설정
    ...
    print("-" * 50)
    print("💡 자연스럽게 대화하시거나, 요금제 관련 질문을 해보세요!")
    print("-" * 50)
        ...
        try:
            ...
            if user_input.lower() == 'users':
                ...
                if 'error' in stats:
                    ...
                    print(f"  - 요금제 추천 대화: {stats['plan_recommendations']}개")
                    print(f"  - 일반 대화: {stats['general_conversations']}개")
            ...
            # 요금제 추천인지 확인하여 표시
            is_recommendation = chatbot.is_plan_recommendation_request(user_input)
