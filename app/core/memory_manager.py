# app/core/memory_manager.py - JSON 직렬화 문제 해결
...
class MemoryManager:
    ...
    def safe_save_json(self, data: Any, file_path: Path) -> bool:
        """안전한 JSON 저장 (Enum 처리 포함)"""
        try:
            # Enum을 문자열로 변환하는 전처리
            cleaned_data = self.clean_data_for_json(data)
            
            with open(file_path, 'w', encoding='utf-8') as f:
                json.dump(cleaned_data, f, ensure_ascii=False, indent=2, cls=CustomJSONEncoder)
            return True
    ...
    def clean_data_for_json(self, obj):
        ...
        if isinstance(obj, dict):
            ...
            # 객체인 경우 딕셔너리로 변환
            return self.clean_data_for_json(obj.__dict__)
        else:
            return obj
    ...
    def save_user_memory(self, username: str, chat_history: List[Dict[str, Any]], 
        ...
        try:
            ...
            # 안전한 JSON 저장 사용
            return self.safe_save_json(user_data, memory_file)
    ...
    def save_chat_history_export(self, username: str, chat_history: List[Dict[str, Any]], 
        ...
        try:
            ...
            # 안전한 JSON 저장 사용
            if self.safe_save_json(export_data, Path(filename)):
