...
class PricingPlanChatbot:
    ...
    def format_chat_history(self) -> str:
        ...
        formatted_history = []
        for entry in self.chat_history[-5:]:  # 최근 5개의 대화만 포함
            formatted_history.append(f"사용자: {entry['human']}")
            formatted_history.append(f"챗봇: {entry['ai']}")
    ...
    def save_chat_history(self, filename: str = None):
        ...
        if filename is None:
            ...
            filename = f"{self.current_user}_chat_export_{timestamp}.json"
...
def main():
    ...
    PRICING_DATA_FILE = "pricing_plans.txt"  # 요금제 정보 파일 경로
